        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER,reg_path) as hkey:
            ...
            for i in range(winreg.QueryInfoKey(hkey)[1]):
                ...
                if name.startswith("Item"):
                    print(f"{name}: {value}")
                    mru_entries.append((version_name, name,value))
                    # count = count + 1
                    count += 1
                    if count >= max_items:
                        break
...
# 결과 저장 - TXT
with open(f"excel_{now_str}.txt","w", encoding="utf-8") as txt_file:
    txt_file.write("엑셀 최근 문서 기록 \n\n")
    for version,item,path in mru_entries:
        txt_file.write(f"[{version}] {item} = {path}\n")

# 결과 저장 - CSV
with open(f"excel_{now_str}.csv","w", newline='', encoding="utf-8-sig") as csv_file:
    writer = csv.writer(csv_file)
    writer.writerow(["Office 버전","항목 이름","파일 경로"])
    writer.writerows(mru_entries)

print(f"\n 결과가 다음과 같이 저장 되었습니다: ")
print(f" - excel_{now_str}.txt")
print(f" - excel_{now_str}.csv")
