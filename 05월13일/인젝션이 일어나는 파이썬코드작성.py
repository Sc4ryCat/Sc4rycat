...
def item_search():
    ...
    """ 
    이렇게 초기화 해도되고 안해도 됨
    search_text 는 처음에 get방식으로 호출되기 때문에 초기화세팅값이 없으면 오류가 나기때문
    search_text = ""
    result_rows = [] -> sql실행결과이며 post 요청일때만 쿼리가 실행되어 존재
    search_sql = ""  ->
    """
    ...
    # search_sql = f"""
    #             select o.member_id, s.food_name, o.buy_count, o.buy_date,
    #             s.price, (s.price * o.buy_count)
    #             total_price from order_record o(nolock)
    #             join supermarket s(nolock)
    #             on s.food_no = o.food_no
